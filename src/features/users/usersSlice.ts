...
import {
  sortedDataByLanguageType,
  sortedDataByRepoNameType,
} from "../../utils/charts";
...
export const selectFollowers = (state: RootState) =>
  state.github_user.followers;
export const selectReposByLanguage = (state: RootState) =>
  state.github_user.repos.reduce(getLanguage, [] as sortedDataByLanguageType[]);
export const selectReposByRepoName = (state: RootState) =>
  state.github_user.repos.reduce(
    getReposNameByStarAndFork,
    [] as sortedDataByRepoNameType[]
  );
...
function getReposNameByStarAndFork(
  arr: sortedDataByRepoNameType[],
  item: defaultRepoType
) {
  ...
  if (name !== null) {
    if (arr.length === 0) {
      ...
      arr.some((item: sortedDataByRepoNameType) => item.name === name)
    ) {
      arr = arr.map((item: sortedDataByRepoNameType) =>
        item.name === name
          ? {
              ...item,
              stars: item.stars + stargazers_count,
              forks: item.forks + forks_count,
            }
          : item
      );
    } else {
...
function getLanguage(arr: sortedDataByLanguageType[], item: defaultRepoType) {
  ...
  if (language !== null) {
    if (arr.length === 0) {
      ...
      arr.some((item: sortedDataByLanguageType) => item.language === language)
    ) {
      arr = arr.map((item: sortedDataByLanguageType) =>
        item.language === language
          ? {
              ...item,
              value: item.value + 1,
              star: item.star + stargazers_count,
            }
          : item
      );
    } else {
